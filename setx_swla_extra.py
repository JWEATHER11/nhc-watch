...
# --- 7-Day Forecast section ------------------------------------------
# HRRR for the short-term days, Euro for the rest, per instruction.
# Grid spans Beaumont, Houston, Lumberton, Silsbee, Woodville, Jasper,
# and Orange -- a real regional blend, not a single point.
...
def fetch_7day_forecast():
    """Days 1-2 from HRRR (its real range), days 3-7 from Euro, all
    blended across the regional grid rather than a single point."""
    ...
    hrrr_url = (
        ...
        f"?latitude={lat_str}&longitude={lon_str}"
        f"&daily=temperature_2m_max,temperature_2m_min,precipitation_sum"
        f"&models=ncep_hrrr_conus&forecast_days=2"
    ...
    euro_url = (
        ...
        f"?latitude={lat_str}&longitude={lon_str}"
        f"&daily=temperature_2m_max,temperature_2m_min,precipitation_sum"
        f"&forecast_days=7&temperature_unit=fahrenheit&_cb={cache_buster}"
    )

    hrrr_data = w._fetch_with_retries_bytes(hrrr_url, "7Day:HRRR")
    euro_data = w._fetch_with_retries_bytes(euro_url, "7Day:Euro")

    hrrr_points = None
    if hrrr_data:
        try:
            parsed = json.loads(hrrr_data.decode("utf-8"))
            hrrr_points = parsed if isinstance(parsed, list) else None
        except (json.JSONDecodeError, UnicodeDecodeError):
            pass
    ...
    if euro_data:
        try:
            parsed = json.loads(euro_data.decode("utf-8"))
            euro_points = parsed if isinstance(parsed, list) else None
        except (json.JSONDecodeError, UnicodeDecodeError):
            pass

    n = len(SEVEN_DAY_POINTS)

    def blend_day(points, day_idx):
        highs, lows, rains = [], [], []
        if not points:
            return None
        for p in points:
            ...
            h = daily.get("temperature_2m_max", [])
            l = daily.get("temperature_2m_min", [])
            r = daily.get("precipitation_sum", [])
            if day_idx < len(h) and h[day_idx] is not None:
                highs.append(h[day_idx])
            if day_idx < len(l) and l[day_idx] is not None:
                lows.append(l[day_idx])
            if day_idx < len(r) and r[day_idx] is not None:
                rains.append(r[day_idx])
        if not highs or not lows:
            return None
        avg_high = round(sum(highs) / len(highs))
        avg_low = round(sum(lows) / len(lows))
        hit_count = sum(1 for v in rains if v >= 2.5)
        rain_pct = round(100 * hit_count / len(rains)) if rains else 0
        return {"high": avg_high, "low": avg_low, "rain_pct": rain_pct}

    days = []
    for day_idx in range(7):
        if day_idx < 2 and hrrr_points:
            result = blend_day(hrrr_points, day_idx)
            if result:
                days.append(result)
                continue
        if euro_points:
            result = blend_day(euro_points, day_idx)
            days.append(result)
        else:
            days.append(None)
...
def build_7day_section(days):
    ...
    for i, d in enumerate(days):
        ...
        lines.append(f"{day_labels[i]}: High {d['high']} / Low {d['low']} / Rain: {d['rain_pct']}% ({cov})")
    return lines
