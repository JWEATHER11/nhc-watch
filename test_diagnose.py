#!/usr/bin/env python3
"""Diagnoses exactly what's triggering is_interesting right now."""
...
for model_key in w.ENSEMBLE_MODELS:
    ...
    _, name = w.ENSEMBLE_MODELS[model_key]
    print(f"{name}: {ensemble_signals[model_key]}")

nhc_summary = w.fetch_nhc_outlook_summary()
print("NHC summary:", repr(nhc_summary))
