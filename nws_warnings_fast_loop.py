#!/usr/bin/env python3
"""
nws_warnings_fast_loop.py -- Continuous near-real-time wrapper around
nws_warnings_pipeline.py, same self-restarting pattern as every other
fast loop in this system. Warnings are life-safety critical, so this
checks every ~25 seconds like everything else.
"""
...
def commit_state_if_changed():
    try:
        subprocess.run(["git", "config", "user.name", "nws-warnings-fast-watch-bot"], check=True)
        subprocess.run(["git", "config", "user.email", "actions@users.noreply.github.com"], check=True)
        subprocess.run(["git", "add", "nws_warnings_state.json"], check=True)
        result = subprocess.run(["git", "diff", "--quiet", "--cached"])
        if result.returncode == 0:
            return
        subprocess.run(["git", "commit", "-m", "Update NWS warnings state [skip ci]"], check=True)
...
def trigger_self_restart():
    ...
    url = f"https://api.github.com/repos/{repo}/actions/workflows/nws-warnings-fast-watch.yml/dispatches"
...
def main():
    print(f"Starting continuous NWS warnings poll loop, checking every {POLL_INTERVAL_SEC}s.")
    iteration = 0
    while True:
        ...
        iteration += 1
        try:
            nws_warnings_pipeline.main()
        except SystemExit as e:
            print(f"[iteration {iteration}] nws_warnings_pipeline.main() exited with code {e.code} -- continuing loop.")
        except Exception as e:
