#!/usr/bin/env python3
"""
nhc_outlook_fast_loop.py -- Continuous near-real-time wrapper around
nhc_outlook_pipeline.py, same self-restarting pattern as the other fast
loops. GitHub's scheduled (cron) trigger proved unreliable for this
workflow -- only 2 organic firings over many hours -- so this replaces
that with a continuous ~25s poll loop that self-restarts before the 6h
job limit, matching the pattern already proven for advisories, recon,
and SPC.
"""
...
def commit_state_if_changed():
    try:
        subprocess.run(["git", "config", "user.name", "nhc-outlook-fast-watch-bot"], check=True)
        subprocess.run(["git", "config", "user.email", "actions@users.noreply.github.com"], check=True)
        subprocess.run(["git", "add", "outlook_state.json"], check=True)
        result = subprocess.run(["git", "diff", "--quiet", "--cached"])
        if result.returncode == 0:
            return
        subprocess.run(["git", "commit", "-m", "Update outlook state [skip ci]"], check=True)
...
def trigger_self_restart():
    ...
    url = f"https://api.github.com/repos/{repo}/actions/workflows/nhc-outlook-fast-watch.yml/dispatches"
...
def main():
    print(f"Starting continuous outlook poll loop, checking every {POLL_INTERVAL_SEC}s.")
    iteration = 0
    while True:
        ...
        iteration += 1
        try:
            nhc_outlook_pipeline.main()
        except SystemExit as e:
            print(f"[iteration {iteration}] nhc_outlook_pipeline.main() exited with code {e.code} -- continuing loop.")
        except Exception as e:
