#!/usr/bin/env python3
"""
wxmodel_fast_loop.py -- Continuous near-real-time wrapper around
wxmodel_pipeline.py, same self-restarting pattern as every other fast
loop in this system.
"""
...
def commit_state_if_changed():
    try:
        subprocess.run(["git", "config", "user.name", "wxmodel-fast-watch-bot"], check=True)
        subprocess.run(["git", "config", "user.email", "actions@users.noreply.github.com"], check=True)
        subprocess.run(["git", "add", "wxmodel_state.json"], check=True)
        result = subprocess.run(["git", "diff", "--quiet", "--cached"])
        if result.returncode == 0:
            return
        subprocess.run(["git", "commit", "-m", "Update WX model state [skip ci]"], check=True)
...
def trigger_self_restart():
    ...
    url = f"https://api.github.com/repos/{repo}/actions/workflows/wxmodel-fast-watch.yml/dispatches"
...
def main():
    print(f"Starting continuous WX model poll loop, checking every {POLL_INTERVAL_SEC}s.")
    iteration = 0
    while True:
